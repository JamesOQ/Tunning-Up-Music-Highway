...
This involved highlighting the crash data we wanted to collect on the dashboards with a selection box and saving the data as a .csv file. Since the "Recent Crashes" webpage would crash itself if too many datapoints were selected at once, and noticing that the stretch of I-40 going through Madison and Henderson counties had an unsually high number of crashes with severe injuries and fatalities given that they are mostly rural, we decided to focus only on the crashes that occured in the aforementioned counties. While it was infeasible to collect only the crashes from the "Recent Crashes" dashboard that happened on I-40, we were able to get all recent crashes in Madison and Henderson counties and resolved to filter for I-40 crashes later. Here, are the two datasets immediately after we collected them from the above dashboards:
